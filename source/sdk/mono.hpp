...
inline std::string read_widechar( const std::uintptr_t address, const std::size_t size ) {
	...
	memory->read_memory( reinterpret_cast<void*>(address), buffer.get( ), size );
...
namespace mono {
	inline mono_root_domain_t* get_root_domain( ) {
		return reinterpret_cast<mono_root_domain_t*>(memory->read<uintptr_t>( g::mono_module /* module handle of mono-2.0-bdwgc.dll */ + 0x499c78 ));
	}
