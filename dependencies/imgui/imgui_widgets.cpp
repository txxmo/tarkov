...
bool ImGui::Checkbox( const char* label, bool* v ) {
    ...
    static std::unordered_map< ImGuiID, checkbox_anim > values;
    auto value = values.find( id );

    if( value == values.end( ) ) {

        values.insert( { id, checkbox_anim( ) } );
        value = values.find( id );
    }

    value->second.fade = ImLerp( value->second.fade, ( *v ? 1.f : 0.f ), 0.1f );
    value->second.check = ImLerp( value->second.check, *v ? 0.f : 2.f, g.IO.DeltaTime * 12.f );

    RenderFrame( frame_bb.Min, frame_bb.Max, hovered ? ImColor( 32, 32, 32 ) : ImColor( 30, 30, 30 ), 0, ROUNDING );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, ImColor( 1.f, 1.f, 1.f, 0.07f * style.Alpha ), ROUNDING );

    RenderFrame( frame_bb.Min, frame_bb.Max, custom.get_accent_color( value->second.fade * style.Alpha ), 0, ROUNDING );

    RenderCheckMark( window->DrawList, frame_bb.GetCenter( ) - ImVec2( ( square_sz / 2 + 1 ) / 2, ( ( square_sz / 2 ) / 2 ) * value->second.check + 4 ), GetColorU32( ImGuiCol_CheckMark, value->second.fade ), square_sz / 2 + 1  );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    window->DrawList->AddRectFilled( clickable.Min, clickable.Max, hovered || popup_open ? ImColor( 32, 32, 32 ) : ImColor( 30, 30, 30 ), 2 );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
    ...
    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, hovered || IsItemActive( ) ? ImColor( 32, 32, 32 ) : ImColor( 30, 30, 30 ), 2 );
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddText( ImVec2( bb.Min.x + 5, bb.GetCenter( ).y - CalcTextSize( label, 0, 1 ).y / 2 ), selected ? custom.get_accent_color( style.Alpha ) : ImColor( 1.f, 1.f, 1.f, selected ? 1.f : 0.5f ), label );
